...
import { Trophy, Star, Medal, Target, RefreshCw, Send, ShieldCheck, Zap, Loader2 } from 'lucide-react';
import { fetchAchievements, fetchAchievementsRealtime } from '../lib/actions';
...
const Achievements: React.FC = () => {
  ...
  React.useEffect(() => {
    const unsub = fetchAchievementsRealtime((data) => {
  ...
  const defaultCategories = [
    { title: 'Keunggulan Akademik', desc: 'Arsip Olimpiade Sains (OSN), Debat, dan Lomba Karya Ilmiah.', icon: <Star className="h-6 w-6" />, color: 'from-blue-500 to-indigo-600' },
    { title: 'Prestasi Olahraga', desc: 'Medali O2SN, Atletik, dan Turnamen Olahraga tingkat Provinsi.', icon: <Trophy className="h-6 w-6" />, color: 'from-orange-400 to-red-600' },
    { title: 'Seni & Budaya', desc: 'Kemenangan FLS2N, Paduan Suara, dan Pelestarian Budaya.', icon: <Medal className="h-6 w-6" />, color: 'from-purple-400 to-pink-600' },
    { title: 'Inovasi & Teknologi', desc: 'Rekam jejak kompetisi Robotik, Coding, dan Kreativitas Digital.', icon: <Zap className="h-6 w-6" />, color: 'from-emerald-400 to-teal-600' }
  ];
  return (
    <section className="py-40 bg-[#F8F9FB] overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-24">
          {achievements.length > 0 ? achievements.slice(0, 8).map((item, idx) => (
            <div key={idx} className="group relative bg-white p-10 rounded-[48px] border border-gray-100 hover:shadow-2xl transition-all duration-700 overflow-hidden">
               <div className="relative z-10">
                 <div className="w-14 h-14 rounded-2xl bg-amber-500/10 flex items-center justify-center text-amber-500 mb-8">
                   <Trophy className="h-6 w-6" />
                 </div>
                 <h4 className="text-xl font-black text-[#0A0F1E] leading-tight mb-3">{item.title}</h4>
                 <p className="text-gray-500 text-xs font-bold uppercase tracking-widest mb-4">{item.recipient_name} — {item.year}</p>
                 <p className="text-gray-600 text-sm font-light leading-relaxed truncate-3-lines">{item.description}</p>
               </div>
            </div>
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <p className="text-gray-600 text-sm font-light leading-relaxed mb-10">
                  {item.desc}
                </p>
                ...
                <div className="pt-8 border-t border-gray-50 flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-[9px] font-black text-gray-300 uppercase tracking-widest">SINKRONISASI 2026</span>
                  </div>
                </div>
              </div>
            </div>
