...
import { Calendar as CalendarIcon, MapPin, Clock, Tag, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react';
...
const AcademicCalendar: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = fetchSchedulesRealtime((data) => {
      // Pastikan data memiliki default untuk demo jika kosong
      if (data.length === 0) {
        setEvents([
          ...
          <div className="lg:col-span-8 bg-[#F8F9FB] rounded-[60px] p-10 border border-gray-50">
             <div className="grid grid-cols-7 gap-4 mb-8">
               {['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'].map(day => (
                 <div key={day} className="text-center text-[10px] font-black uppercase tracking-[0.3em] text-gray-600">{day}</div>
               ))}
             </div>
            ...
            {events.map((ev, i) => (
              <div key={i} className="group flex items-start space-x-6 p-8 bg-white rounded-[40px] border border-gray-100 hover:shadow-2xl hover:border-[#F3C623]/30 transition-all cursor-default">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-[10px] font-medium flex items-center">
