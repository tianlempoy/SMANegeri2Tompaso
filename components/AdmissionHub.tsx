...
import { ClipboardList, Calendar, CheckCircle, FileText, ArrowRight, Phone, Lock } from 'lucide-react';
...
const AdmissionHub: React.FC<AdmissionHubProps> = ({ onRegisterClick }) => {
    ...
    <div className="pt-40 pb-32 bg-white min-h-screen">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-24">
          <span className="text-[#F3C623] font-black tracking-[0.5em] uppercase text-xs mb-6 block">Pusat Penerimaan {getPPDBYear()}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 mb-32">
          ...
          <div className="flex flex-col justify-center space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <button 
                 onClick={isPPDBOpen ? onRegisterClick : undefined}
                 disabled={!isPPDBOpen}
                 className={`p-10 rounded-[40px] font-black text-xs uppercase tracking-widest transition-all shadow-xl text-center flex items-center justify-center space-x-2 ${
                   isPPDBOpen 
                   ? 'bg-[#F3C623] text-[#0A0F1E] hover:scale-105 shadow-yellow-500/20' 
                   : 'bg-gray-200 text-gray-500 cursor-not-allowed opacity-80'
                 }`}
               >
                 {!isPPDBOpen && <Lock className="w-4 h-4" />}
                 <span>{isPPDBOpen ? 'Daftar Sekarang' : 'Pendaftaran Ditutup'}</span>
               </button>
