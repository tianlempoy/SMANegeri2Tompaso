...
const EkskulData = [
  { name: 'Robotika', cat: 'TEKNOLOGI', icon: <Code />, color: 'bg-blue-500' },
  { name: 'Pramuka Inti', cat: 'KEPEMIMPINAN', icon: <Heart />, color: 'bg-red-500' },
  { name: 'Seni Tari Tompaso', cat: 'SENI', icon: <Palette />, color: 'bg-purple-500' },
  { name: 'Basket Team', cat: 'OLAHRAGA', icon: <Bike />, color: 'bg-orange-500' },
  { name: 'Multimedia & TV', cat: 'TEKNOLOGI', icon: <Camera />, color: 'bg-indigo-500' },
  { name: 'Paduan Suara', cat: 'SENI', icon: <Music />, color: 'bg-pink-500' },
  { name: 'Klub Bahasa Inggris', cat: 'AKADEMIK', icon: <Trophy />, color: 'bg-emerald-500' },
  { name: 'E-Sport Hub', cat: 'TEKNOLOGI', icon: <Monitor />, color: 'bg-slate-700' },
];

const Extracurriculars: React.FC = () => {
  const [filter, setFilter] = useState('SEMUA');
  const categories = ['SEMUA', 'TEKNOLOGI', 'OLAHRAGA', 'SENI', 'KEPEMIMPINAN'];

  const filtered = filter === 'SEMUA' ? EkskulData : EkskulData.filter(e => e.cat === filter);
            ...
            {categories.map(c => (
              <button 
                ...
                className={`px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${
                  filter === c ? 'bg-[#0A0F1E] text-[#F3C623]' : 'bg-gray-50 text-black hover:bg-gray-100'
                }`}
              >
                {c}
              </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {filtered.map((e, i) => (
            <div key={i} className="group relative bg-[#F8F9FB] rounded-[48px] p-10 overflow-hidden border border-gray-50 hover:shadow-2xl transition-all duration-700">
              ...
              <p className="text-gray-600 text-[10px] font-bold uppercase tracking-widest">{e.cat}</p>
