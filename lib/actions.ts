...
import { NewsItem, Page, Announcement, Teacher, OSISMember, Activity, Achievement, GalleryImage, Policy, PPDBRegistration } from '../types.js';
...
export const apiGetActivityLog = async (options?: { limit?: number }) => ({ data: [] });

export const apiGetUsers = async () => ({ data: getFromStorage(STORAGE_KEYS.USERS) });

export const apiCreateUser = async (user: any) => {
  const items = getFromStorage(STORAGE_KEYS.USERS);
...
export const apiUpdateUser = async (id: any, data: any) => {
  const items = getFromStorage(STORAGE_KEYS.USERS);
  const idx = items.findIndex((u: any) => u.id === id.toString());
  if (idx >= 0) {
...
export const apiDeleteUser = async (id: any) => {
  const items = getFromStorage(STORAGE_KEYS.USERS);
  saveToStorage(STORAGE_KEYS.USERS, items.filter((u: any) => u.id !== id.toString()));
  return { success: true };
};

export const apiResetPassword = async (id: any, newPassword: string) => {
  const items = getFromStorage(STORAGE_KEYS.USERS);
  const idx = items.findIndex((u: any) => u.id === id.toString());
  if (idx >= 0) {
